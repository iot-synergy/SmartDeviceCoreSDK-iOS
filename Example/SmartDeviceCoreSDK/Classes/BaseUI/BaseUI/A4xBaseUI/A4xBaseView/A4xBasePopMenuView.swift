...
protocol A4xBasePopMenuViewDelegate: NSObjectProtocol {
    func A4xBasePopMenuViewDidSelectIndex(index: Int)
}
...
public enum A4xBasePopMenuViewConfigure {
    ...
    case popMenuWidth(CGFloat)              //菜单宽度
}

public class A4xBasePopMenuView: UIView {
    
    //delegate
    weak var delegate: A4xBasePopMenuViewDelegate?
    //block
    public var didSelectMenuBlock: ((_ index: Int) -> Void)?
    
    public var clicktMenuBlock: ((_ dismiss: Int) -> Void)?
    ...
    //icon左间距
    private var popMenuIconLeftMargin: CGFloat = 11
    //菜单与屏幕边距
    private var popMenuMargin: CGFloat = 10
    //区域外背景透明度
    private var popMenuWidth: CGFloat = 185
    ...
    static let cellID: String = "A4xBasePopMenuViewCellID"
    ...
    public init(menuWidth: CGFloat, arrow: CGPoint, datas: [(icon: String, title: String)], configures: [A4xBasePopMenuViewConfigure] = []) {
    ...
    func initViews() {
        ...
        tableView.register(A4xBasePopMenuViewCell.classForCoder(), forCellReuseIdentifier: A4xBasePopMenuView.cellID)
...
extension A4xBasePopMenuView : UITableViewDataSource,UITableViewDelegate{
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if popData.count > indexPath.row {
            let cell = tableView.dequeueReusableCell(withIdentifier: A4xBasePopMenuView.cellID) as! A4xBasePopMenuViewCell
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if self.delegate != nil{
            self.delegate?.A4xBasePopMenuViewDidSelectIndex(index: indexPath.row)
        }
...
class A4xBasePopMenuViewCell: UITableViewCell {
    ...
    func fill(iconName: String, title: String, islast: Bool = false) {
        iconImage.image = bundleImageFromImageName(iconName)?.rtlImage()
        lblTitle.text = title
    ...
    override func layoutSubviews() {
        ...
        self.iconImage.resetFrameToFitRTL()
        self.lblTitle.frame = CGRect(x: 15 + iconLeft * 2, y: 0, width: self.bounds.size.width - 50, height: self.bounds.size.height)
        self.lblTitle.resetFrameToFitRTL()
