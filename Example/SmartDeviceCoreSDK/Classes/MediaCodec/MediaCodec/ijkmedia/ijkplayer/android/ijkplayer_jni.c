...
IjkMediaPlayer_native_setLogLevel(JNIEnv *env, jclass clazz, jint level)
{
    ...
    //a4x:
    ijkmp_global_set_log_report(true);
    ALOGD("moncleanup\n");
}

//a4x:
static jobject g_jLogCallback;

static void _logCallback(int level, char *tag, char* log) {
    JNIEnv *jniEnv;
    int state = (*g_jvm)->GetEnv(g_jvm, (void**)&jniEnv, JNI_VERSION_1_4);
    if (state !=JNI_OK) {
        (*g_jvm)->AttachCurrentThread(g_jvm, &jniEnv, NULL);
    }

    jclass jclsCB = (*jniEnv)->GetObjectClass(jniEnv, g_jLogCallback);
    if (jclsCB == NULL) {
        return;
    }

    jmethodID jmidCB =
        (*jniEnv)->GetMethodID(jniEnv, jclsCB, "onLogCallback", "(ILjava/lang/String;Ljava/lang/String;)V");
    if (jmidCB == NULL) {
        return;
    }

    jstring jtag = (*jniEnv)->NewStringUTF(jniEnv, tag);
    jstring jlog = (*jniEnv)->NewStringUTF(jniEnv, log);

    jclass playerCallbackCls = (*jniEnv)->GetObjectClass(jniEnv, g_jLogCallback);

    //call java method
    //jmethodID logCallback = (*jniEnv)->GetMethodID(jniEnv, playerCallbackCls, "onLogCallback",
    //                                             "(ILjava/lang/String;Ljava/lang/String;)V");

    (*jniEnv)->CallVoidMethod(jniEnv, g_jLogCallback, jmidCB, level, jtag, jlog);
...
static void
IjkMediaPlayer_native_setLogCallback(JNIEnv *env, jclass clazz, jobject jcallback)
{
    MPTRACE("%s(%p)\n", __func__, jcallback);
    if (g_jvm == NULL) {
        return ;
    ...
    g_jLogCallback = (*env)->NewGlobalRef(env, jcallback);;
    ijkmp_global_set_log_callback(_logCallback);
...
static void _videoFrameCallback(void *opaque, int w, int h, char *data, int data_len)
{
    JNIEnv *jniEnv;
    int state = (*g_jvm)->GetEnv(g_jvm, (void**)&jniEnv, JNI_VERSION_1_4);
    if (state != JNI_OK) {
    ...
    jobject thiz = (jobject)opaque;
    if (thiz == NULL) {
        MPTRACE("%s thiz is NULL\n", __func__);
        return;
    }

    jclass clz = (*jniEnv)->GetObjectClass(jniEnv, thiz);
    if (clz == NULL) {
        MPTRACE("%s clz is NULL\n", __func__);
        return;
    }

    jmethodID jmidCB =
        (*jniEnv)->GetMethodID(jniEnv, clz, "onVideoFrame", "(II[B)V");
    if (jmidCB == NULL) {
        MPTRACE("%s jmidCB is NULL\n", __func__);
        return;
    }

    jbyteArray array = (*jniEnv)->NewByteArray(jniEnv, data_len);
    if (array == NULL) {
        MPTRACE("%s array is NULL\n", __func__);
        return;
    }
    (*jniEnv)->SetByteArrayRegion(jniEnv, array, 0, data_len, (jbyte *)data);

    (*jniEnv)->CallVoidMethod(jniEnv, thiz, jmidCB, w, h, array);

    (*jniEnv)->ReleaseByteArrayElements(jniEnv, array, (*jniEnv)->GetByteArrayElements(jniEnv, array, NULL), JNI_ABORT);
...
static void
IjkMediaPlayer_native_setVideoProcessEnable(JNIEnv *env, jobject thiz, jboolean enable)
{
    MPTRACE("%s: %d\n", __func__, enable);
    jint retval = 0;
    void * old = NULL;
    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
    JNI_CHECK_GOTO(mp, env, "java/lang/IllegalStateException", "mpjni: native_setVideoProcessEnable: null mp", LABEL_RETURN);

    if (enable == JNI_TRUE) {
        jobject x = (*env)->NewGlobalRef(env, thiz);
        old = ijkmp_set_video_frame_callback(mp, _videoFrameCallback, (void *)x);
    } else {
        old = ijkmp_set_video_frame_callback(mp, NULL, NULL);
    }

    if (old != NULL) {
        (*env)->DeleteGlobalRef(env, (jobject)old);
    }
...
static void
IjkMediaPlayer_native_putVideoFrame(JNIEnv *env, jobject thiz, jint w, jint h, jbyteArray data)
{
    MPTRACE("%s: w: %d h: %d\n", __func__, w, h);
    jint retval = 0;

    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
    JNI_CHECK_GOTO(mp, env, "java/lang/IllegalStateException", "mpjni: native_putVideoFrame: null mp", LABEL_RETURN);
    ...
    if (w > 0 && h > 0 && data != NULL && len > 0) {
        char *carray = (char *)((*env)->GetByteArrayElements(env, data, NULL));
        ijkmp_put_video_frame(mp, (int)w, (int)h, carray, (int)len);
    } else {
        ijkmp_put_video_frame(mp, 0, 0, NULL, 0);
    }
...
//-----add record end--------
    ...
    { "native_setLogCallback",  "(Ltv/danmaku/ijk/media/player/IjkMediaPlayer$NativeLogCallback;)V",                  (void *) IjkMediaPlayer_native_setLogCallback },


    { "native_setVideoProcessEnable", "(Z)V", (void *)IjkMediaPlayer_native_setVideoProcessEnable },
    { "native_putVideoFrame", "(II[B)V", (void *)IjkMediaPlayer_native_putVideoFrame },
