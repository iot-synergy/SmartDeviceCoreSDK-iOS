...
static void video_image_display2(FFPlayer *ffp)
{
    ...
    if (ffp->vfc != NULL && vp->updated == 0) {
        av_log(ffp, AV_LOG_DEBUG, "frame %"PRId64"not updated!\n", vp->pos);
        return;
    }
...
static double get_clock(Clock *c)
{
    ...
    if (c->paused) {
        ...
        #if 0
        if (time - c->last_updated >= 1.0) {
            val = c->pts - (time - c->last_updated) * (1.0 - c->speed);
        } else {
            val = c->pts_drift + time - (time - c->last_updated) * (1.0 - c->speed);
        }
        #endif
        val = c->pts - (time - c->last_updated) * (1.0 - c->speed);
        //av_log(NULL, AV_LOG_INFO, "####get_clock, clock: %p c.pts:%f, pts_drift:%f, time:%f, last_updated:%f, speed:%f val: %f", c, c->pts, c->pts_drift, time, c->last_updated, c->speed, val);
        return val;
    }
}
...
{
    //av_log(NULL, AV_LOG_INFO, "####set_clock_at, clock:0x%x, time:%f, pts:%f", c, time, pts);
    c->pts = pts;
    c->last_updated = time;
...
static int get_master_sync_type(VideoState *is) {
    // ALOGD("####get_master_sync_type, is->av_sync_type:%d, is->video_st:0x%x", is->av_sync_type, is->video_st);
...
static double get_master_clock(VideoState *is)
{
    ...
    //av_log(NULL, AV_LOG_INFO, "####get_master_clock, vidclk:0x%x, sync:%d, clock:%f", &(is->vidclk), get_master_sync_type(is), val);
    return val;
}
...
static void update_video_pts(VideoState *is, double pts, int64_t pos, int serial) {
    av_log(NULL, AV_LOG_INFO, "####update_video_pts, is:%p, pts:%f, pos:%ld, serial:%d", is, pts, pos, serial);
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            //av_log(NULL, AV_LOG_DEBUG, "###video_refresh, update_video_pts, vp->pts:%lf", vp->pts);
            if (!isnan(vp->pts))
...
{
    av_log(NULL, AV_LOG_INFO, "####queue_picture, pts:%f, pos:%"PRId64"\n", pts, pos);
    VideoState *is = ffp->is;
    Frame *vp;
    ...
    if (ffp->enable_accurate_seek && is->video_accurate_seek_req && !is->seek_req) {
        if (!isnan(pts)) {
            ...
            if (((pts * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) && (pts * 1000 * 1000 < is->seek_pos)) {
                ...
                while (is->audio_accurate_seek_req && !is->abort_request) {
                    ...
                    av_log(NULL, AV_LOG_INFO, "####queue_picture, accurate_seek_aframe_pts:%ld"
                    " accurate_seek_vframe_pts:%ld "
                    " deviation2:%ld"
                    " deviation3:%ld"
                    " is->seek_pos:%"PRId64"pts:%f", is->accurate_seek_aframe_pts, is->accurate_seek_vframe_pts,
                    deviation2, deviation3, is->seek_pos, pts);
                ...
                if ((now - is->accurate_seek_start_time) <= ffp->accurate_seek_timeout) {
                    av_log(NULL, AV_LOG_WARNING, "####accurate_seek_timeout return");
    ...
    if (!(vp = frame_queue_peek_writable(&is->pictq))) {
        av_log(ffp, AV_LOG_INFO, "####frame_queue_peek_writable failed!\n");
        return -1;
    }
    ...
    if (vp->bmp) {
        ...
        if (ffp->vfc != NULL) {
            pthread_mutex_lock(&ffp->vfc_mutex);
            if (ffp->overlay_format == SDL_FCC_RV32) {
                // Android RGBX
                int width = vp->bmp->w;
                int height = vp->bmp->h;
                ffp->vfc(ffp->vfc_opaque, width, height, (char *)vp->bmp->pixels[0], vp->bmp->pitches[0] * height);
                vp->updated = 0;
                av_log(ffp, AV_LOG_DEBUG, "frame %"PRId64"callback!\n", vp->pos);
            } else if (ffp->overlay_format == SDL_FCC__GLES2) {
                // iOS YUV420P
                int width = vp->bmp->w;
                int height = vp->bmp->h;
                
                int stridey = vp->bmp->pitches[0];
                int strideuv = height;
                // I420P
                if (ffp->yuvPlane == NULL) {
                    ffp->yuvPlane = (uint8_t*)malloc(stridey * strideuv * 3/2);
                }
                
                if (ffp->yuvPlane == NULL) {
                    av_log(ffp, AV_LOG_ERROR, "bad malloc");
                    return -1;
                }
                memcpy(ffp->yuvPlane, vp->bmp->pixels[0], stridey * strideuv);
                memcpy(ffp->yuvPlane + stridey * strideuv, vp->bmp->pixels[1], stridey * strideuv/4);
                memcpy(ffp->yuvPlane + stridey * strideuv + stridey * strideuv/4, vp->bmp->pixels[2], stridey * strideuv/4);
                ffp->vfc(ffp->vfc_opaque, width, height, (char *)ffp->yuvPlane, stridey * strideuv * 3/2);
                vp->updated = 0;
                
                av_log(ffp, AV_LOG_DEBUG, "width: %d height: %d linesize: %d, pitches[1]:%d, pitches[2]:%d\n", width, height, vp->bmp->pitches[0], vp->bmp->pitches[1], vp->bmp->pitches[2]);
                av_log(ffp, AV_LOG_DEBUG, "frame %"PRId64"callback!\n", vp->pos);
            }
            else {
                av_log(ffp, AV_LOG_FATAL, "ffp->overlay_format: 0x%x not support!\n", ffp->overlay_format);
                vp->updated = 1;
            }
            pthread_mutex_unlock(&ffp->vfc_mutex);
        }
...
static int get_video_frame(FFPlayer *ffp, AVFrame *frame)
{
    ...
    if ((got_picture = decoder_decode_frame(ffp, &is->viddec, frame, NULL)) < 0) {
        av_log(NULL, AV_LOG_ERROR, "####decoder_decode_frame failed\n");
        return -1;
    }
        ...
        if (got_frame) {
                ...
                if (ffp->enable_accurate_seek && is->audio_accurate_seek_req && !is->seek_req) {
                    ...
                    if (!isnan(frame_pts)) {
                        ...
                        if (((audio_clock * 1000 * 1000 < is->seek_pos ) || deviation > MAX_DEVIATION) && (audio_clock * 1000 * 1000 < is->seek_pos)) {
                            ...
                            while (is->video_accurate_seek_req && !is->abort_request) {
                                ...
                                av_log(NULL, AV_LOG_INFO, "####audio_thread, accurate_seek_aframe_pts:%ld"
                                    " accurate_seek_vframe_pts:%ld"
                                    " deviation2:%ld"
                                    " deviation3:%ld"
                                    " is->seek_pos:%"PRId64"\n", is->accurate_seek_aframe_pts, is->accurate_seek_vframe_pts,
                                    deviation2, deviation3, is->seek_pos);
                            ...
                            if (audio_seek_pos == is->seek_pos) {
                                av_log(NULL, AV_LOG_INFO, "audio accurate_seek is ok, is->drop_aframe_count=%d, audio_clock = %lf seek_pos: %"PRId64"\n", is->drop_aframe_count, audio_clock, is->seek_pos);
...
static int ffplay_video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (ffp->get_frame_mode) {
            ...
            pts = pts * 1000;
            //av_log(NULL, AV_LOG_DEBUG, "###ffplay_video_thread, frame->pts:%lld, pts:%lf, dst_pts:%lld", frame->pts, pts, dst_pts);
            if (pts >= dst_pts) {
        ...
        while (ret >= 0) {
            ...
            //av_log(NULL, AV_LOG_ERROR, "###get_video_frame, dur: %f picq in %f, frame->pts:%ld\n", duration, pts, frame->pts);
...
{
    av_log(NULL, AV_LOG_TRACE, "####synchronize_audio, sync_type:%d", get_master_sync_type(is));
...
{
    av_log(NULL, AV_LOG_WARNING, "####stream_component_open, stream_index:%d\n", stream_index);
    VideoState *is = ffp->is;
    AVFormatContext *ic = is->ic;
...
static void control_video_queue_duration(FFPlayer *ffp, VideoState *is) {
    ...
    if (cached_duration > is->max_cached_duration) {
        // drop
        av_log(NULL, AV_LOG_INFO, "233 video cached_duration = %"PRId64", nb_packets = %d.\n", cached_duration, nb_packets);
...
static void control_audio_queue_duration(FFPlayer *ffp, VideoState *is) {
    ...
    if (cached_duration > is->max_cached_duration) {
        // drop
        av_log(NULL, AV_LOG_INFO, "233 audio cached_duration = %"PRId64", nb_packets = %d.\n", cached_duration, nb_packets);
...
static int read_thread(void *arg)
{
    ...
    //av_log(NULL, AV_LOG_WARNING, "chen:avformat_open_input after, filename=%s, ffp->av_sync_type:%d, ic->start_time:%lld, //ffp->iformat_name:%s\n", is->filename, ffp->av_sync_type, ic->start_time, ffp->iformat_name);
    if (err < 0 || ic == NULL) {
        if (av_stristart(is->filename, "rtmp", NULL) ||
            av_stristart(is->filename, "rtsp", NULL)) {
    ...
    //av_log(ffp, AV_LOG_INFO, "max_frame_duration: %.3f, ffp->start_time:%lld\n", is->max_frame_duration, ffp->start_time);
    ...
    if (ffp->start_time != AV_NOPTS_VALUE) {
        ...
        //av_log(NULL, AV_LOG_DEBUG, "###avformat_seek_file, timestamp:%lld, ic->start_time:%lld, ffp->start_time:%lld", timestamp, ic->start_time, ffp->start_time);
...
//    is->realtime = is_realtime(ic);
    ...
    double video_stream_start_time = 0.0;
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        if (type == AVMEDIA_TYPE_VIDEO) {
            video_stream_start_time = av_q2d(st->time_base) * st->start_time * 1000000.0;
    ...
    // a4x:向视频时间戳对齐-AVFormatContext->start_time默认取音视频第一帧最小时间戳,当按视频同步，音视频时间戳相差太大，就会出现
    // 进度条播放调到异常位置
    if (ffp->av_sync_type == AV_SYNC_VIDEO_MASTER && video_stream_start_time != AV_NOPTS_VALUE) {
        //av_log(NULL, AV_LOG_DEBUG, "###read_thread, video_stream_start_time:%lf", video_stream_start_time);
        ic->start_time = video_stream_start_time;
    }
    ...
    for (;;) {
        ...
        if (pkt->stream_index >= 0 && pkt->stream_index < is->ic->nb_streams &&
            ...
            if (!ffp->is_first) {
                ...
                //av_log(ffp, AV_LOG_DEBUG, "###set start video start_pts: %"PRId64"+++++ start_dts: %"PRId64"\n", ffp->video_start_dts, ffp->video_start_dts);
            }
        }
...
//      of the seek_pos/seek_rel variables
            ...
            av_log(NULL, AV_LOG_ERROR, "####seek_min: %"PRId64"seek_target: %"PRId64"seek_max: %"PRId64"flags: 0x%x\n", seek_min, seek_target, seek_max, is->seek_flags);
        ...
        if ((av_stristart(ffp->input_filename, "rtsp", NULL) ||
              ...
              av_stristart(ffp->input_filename, "http", NULL) ||
              av_stristart(ffp->input_filename, "https", NULL)) &&
            pkt->stream_index == is->video_stream) {
            av_log(NULL, AV_LOG_INFO, "###read_thread, currentVideoPts %"PRId64"\n", pkt->pts);
...
#pragma  mark - 录制插入
        if (ffp->is_record) { // 可以录制时，写入文件
            ...
            if (ffp->is_first_keyframe_recorded) {
                if (0 != ffp_record_file(ffp, pkt)) {
                    av_log(NULL, AV_LOG_ERROR, "ffp_stop_recording_l pts: %"PRId64" dts: %"PRId64"\n", pkt->pts, pkt->dts);
                    ffp->record_error = 1;
                    ffp->is_record = 0;
                    ffp_stop_record(ffp);
        ...
        if (pkt->stream_index == is->audio_stream && pkt_in_play_range) {
            //av_log(NULL, AV_LOG_WARNING, "chen:queue audio %"PRId64"\n", pkt->pts);
            packet_queue_put(&is->audioq, pkt);
        } else if (pkt->stream_index == is->video_stream && pkt_in_play_range
                   && !(is->video_st && (is->video_st->disposition & AV_DISPOSITION_ATTACHED_PIC))) {
            //av_log(NULL, AV_LOG_WARNING, "chen:queue video %"PRId64"\n", pkt->pts);
            ...
            //av_log(NULL, AV_LOG_ERROR, "vq in %"PRId64"\n", pkt->pts);

        } else if (pkt->stream_index == is->subtitle_stream && pkt_in_play_range) {
            packet_queue_put(&is->subtitleq, pkt);
        } else {
            //av_log(NULL, AV_LOG_WARNING, "chen:queue unref %"PRId64"\n", pkt->pts);
            av_packet_unref(pkt);
        }
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat)
{
    ...
    return is;
fail:
    av_log(NULL, AV_LOG_ERROR, "####stream_open failed");
    is->initialized_decoder = 1;
...
//a4x:
static ijk_log_callback s_log_callback;
void ffp_global_set_log_callback(ijk_log_callback callback)
{
...
void a4x_printf(const char *format, ...) {
    if (s_log_callback != NULL) {
        ...
        va_end(ap);
        if (s_log_callback != NULL) {
            s_log_callback(IJK_LOG_VERBOSE, IJK_LOG_TAG, line);
        }
    }
}
...
static void ffp_log_callback_brief(void *ptr, int level, const char *fmt, va_list vl)
{
    ...
    if (s_log_callback != NULL) {
        ...
        va_end(vl2);
        s_log_callback(ffplv, IJK_LOG_TAG, line);
    }
}
...
FFPlayer *ffp_create()
{
    ...
    ffp->vfc = NULL;
    ffp->vfc_opaque = NULL;
    pthread_mutex_init(&ffp->vfc_mutex, NULL);
    ffp->yuvPlane = NULL;
...
{
    av_log(NULL, AV_LOG_WARNING, "####ffp_destroy");
    if (!ffp)
        return;
    ...
    ffp->vfc = NULL;
    ffp->vfc_opaque = NULL;
    pthread_mutex_destroy(&ffp->vfc_mutex);
    if (ffp->yuvPlane != NULL) {
        free(ffp->yuvPlane);
        ffp->yuvPlane = NULL;
    }
    
    if (ffp->renderBuf != NULL) {
        free(ffp->renderBuf);
        ffp->renderBuf = NULL;
    }
...
long ffp_get_current_position_l(FFPlayer *ffp)
{
    ...
    av_log(NULL, AV_LOG_INFO, "####ffp_get_current_position_l, pos:%lld, start_diff:%lld, is->ic:%p", pos, start_diff, is->ic);
            ...
            if (!ffp->is_first) { // 录制的第一帧，时间从0开始
                ...
                if (pkt->stream_index >= 0 && pkt->stream_index < ffp->m_ofmt_ctx->nb_streams) {
                    if (ffp->m_ofmt_ctx->streams[pkt->stream_index]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
                        ...
                        //av_log(ffp, AV_LOG_DEBUG, "### audio pts: %"PRId64"dts: %"PRId64"ffp->audio_start_pts: %"PRId64"ffp->audio_start_dts: %"PRId64"\n",
                        ...
                        //av_log(ffp, AV_LOG_DEBUG, "### video pts: %"PRId64"dts: %"PRId64"ffp->video_start_pts: %"PRId64"ffp->video_start_dts: %"PIRd64"\n",
...
void *ffp_set_video_frame_callback(FFPlayer *ffp, ijk_video_frame_callback cb, void * opaque)
{
    assert(ffp);
    void *old = NULL;
    pthread_mutex_lock(&ffp->vfc_mutex);
    ...
    ffp->vfc = cb;
    ffp->vfc_opaque = opaque;
    pthread_mutex_unlock(&ffp->vfc_mutex);

    return old;
}

void ffp_put_video_frame(FFPlayer *ffp, int w, int h, char *data, int len)
{
    Frame *vp = NULL;
    AVFrame *old_frame = NULL;
    AVFrame *frame = NULL;
    int src_format = -1;
    int dst_format = -1;
    assert(ffp);

    av_log(ffp, AV_LOG_DEBUG, "%s w: %d h: %d len: %d\n", __func__, w, h, len);

    // pthread_mutex_lock(&ffp->vfc_mutex);
    if (ffp->vfc != NULL) {
        if (frame_queue_nb_remaining(&ffp->is->pictq) > 0) {
            if (ffp->overlay_format == SDL_FCC_RV32) {
                src_format = AV_PIX_FMT_ARGB;
                dst_format = AV_PIX_FMT_0BGR32;
                
                if (ffp->renderBuf == NULL) {
                    ffp->renderBuf = (uint8_t*)malloc(len);
                }
            } else if (ffp->overlay_format == SDL_FCC__GLES2) {
                ...
                if (ffp->renderBuf == NULL) {
                    ffp->renderBuf = (uint8_t*)malloc(w * h * 3/2);
                }
            } else {
                av_log(ffp, AV_LOG_ERROR, "ffp->overlay_format: 0x%x not support", ffp->overlay_format);
                goto end;
            }
            ...
            memcpy(ffp->renderBuf, data, len);

            vp = frame_queue_peek(&ffp->is->pictq);
            old_frame = vp->frame;
#if 0
            av_log(ffp, AV_LOG_DEBUG, "frame serial: %d pts: %f duration: %f pos: %"PRId64"width: %d height: %d format: %s overlay_format: %d %s -> %s\n",
                    vp->serial, vp->pts, vp->duration, vp->pos, vp->width, vp->height, av_get_pix_fmt_name(vp->format), ffp->overlay_format,
                    av_get_pix_fmt_name(src_format), av_get_pix_fmt_name(dst_format));
            av_log(ffp, AV_LOG_DEBUG, "old bmp w: %d h: %d format: %u planes: %d sar_num: %d sar_den: %d pitches[0]: %u\n", vp->bmp->w, vp->bmp->h,
                    vp->bmp->format, vp->bmp->planes, vp->bmp->sar_num, vp->bmp->sar_den,
                    vp->bmp->pitches[0]);

            av_log(ffp, AV_LOG_DEBUG, "old frame linesize: %d width: %d height: %d format: %s pts: %ld flags: %d color_range: %d colorspace: %d color_primaries: %d color_trc: %d\n", old_frame->linesize[0], old_frame->width, old_frame->height,
                   av_get_pix_fmt_name(old_frame->format),  old_frame->pts, old_frame->flags, old_frame->color_range, old_frame->colorspace, old_frame->color_primaries, old_frame->color_trc);
#endif

            if (w > 0 && h > 0 && len > 0) {
                frame = av_frame_alloc();
                if (frame == NULL) {
                    av_log(ffp, AV_LOG_ERROR, "av_frame_alloc failed!\n");
                    goto end;
                }
                ...
                if (av_frame_get_buffer(frame, 1) < 0) {
                    av_log(ffp, AV_LOG_ERROR, "av_frame_get_buffer failed!\n");
                    goto end;
                }

                if (av_image_fill_arrays(frame->data, frame->linesize, (uint8_t *)ffp->renderBuf, src_format, w, h, 1) < 0) {
                    av_log(ffp, AV_LOG_ERROR, "av_image_fill_arrays failed!\n");
                    goto end;
                }
...
#if 0
                av_log(ffp, AV_LOG_DEBUG, "new frame linesize: %d width: %d height: %d format: %s pts: %ld flags: %d color_range: %d colorspace: %d color_primaries: %d color_trc: %d\n",
                        frame->linesize[0], frame->width, frame->height, av_get_pix_fmt_name(frame->format),  frame->pts, frame->flags, frame->color_range, frame->colorspace, frame->color_primaries, frame->color_trc);
#endif

                SDL_VoutLockYUVOverlay(vp->bmp);

                if (SDL_VoutFillFrameYUVOverlay(vp->bmp, frame) < 0) {
                    av_log(NULL, AV_LOG_FATAL, "SDL_VoutFillFrameYUVOverlay failed!\n");
                    goto end;
                }
...
#if 0
                av_log(ffp, AV_LOG_DEBUG, "new bmp w: %d h: %d format: %u planes: %d sar_num: %d sar_den: %d pitches[0]: %u\n",
                        vp->bmp->w, vp->bmp->h, vp->bmp->format, vp->bmp->planes, vp->bmp->sar_num, vp->bmp->sar_den,
                        vp->bmp->pitches[0]);
#endif
            ...
            vp->updated = 1;
            av_log(ffp, AV_LOG_DEBUG, "frame %"PRId64"updated\n", vp->pos);
        }
    }
