...
@synthesize magicPixEnable = _magicPixEnable;
                   ...
                   withOptions:(IJKFFOptions *)options
{
    ...
    if (self) {
        ...
        // a4x: ijkplayer log callback
        [IJKFFMoviePlayerController setLogCallback];
#else
...
- (void)prepareToPlay
{
    ...
    NSLog(@"###prepareToPlay, url:%@", _urlString);
...
{
    NSLog(@"###setLogLevel, logLevel:%ld", logLevel);
    ijkmp_global_set_log_level(logLevel);
}

void nativeLogCallback(int level, char* tag, char* log) {
    NSLog(@"###%s [%d]: %s", tag, level, log);
}

+ (void)setLogCallback
{
    NSLog(@"###setLogCallback");
    ijkmp_global_set_log_callback(nativeLogCallback);
}
...
- (IJKMPMoviePlaybackState)playbackState
{
    ...
    NSLog(@"###playbackState, state:%d", state);
    switch (state) {
...
- (NSTimeInterval)currentPlaybackTime
{
    ...
    NSLog(@"###currentPlaybackTime, current_position:%f", ret);
...
- (void)postEvent: (IJKFFMoviePlayerMessage *)msg
{
    ...
    NSLog(@"###postEvent, msg.what:%d, arg1:%d, arg2:%d", avmsg->what, avmsg->arg1, avmsg->arg2);
    switch (avmsg->what) {
        case FFP_MSG_FLUSH:
            break;
...
- (void)applicationWillResignActive
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (_pauseInBackground && _isLivePlayer) {
            [self pause];
        }
    });
}
...
// video enhance
static void enhanceVideoFrameCallback(void *opaque, int w, int h, char* data, int data_len) {
    NSLog(@"enhanceVideoFrameCallback, data len:%d", data_len);
    IJKFFMoviePlayerController* player = (__bridge IJKFFMoviePlayerController*)opaque;
    if (!player) {
        NSLog(@"IJKFFMoviePlayerController, opaque is nil");
        return ;
    }
    
    @autoreleasepool {
        if (player->_videoDelegate) {
            //NSData* nsFrameData = [NSData dataWithBytesNoCopy:data length:data_len];
            //NSData* nsFrameData = [NSData dataWithBytes:data length:data_len];
            [player->_videoDelegate didEnhanceVideoFrame:w h:h frame:(uint8_t*)data len:data_len];
        }
    }
}

- (void) setMagicPixEnable:(BOOL)magicPixEnable {
    NSLog(@"IJKFFMoviePlayerController, setAiEnable, magicPixEnable:%d", magicPixEnable);
    if (magicPixEnable) {
        ijkmp_set_video_frame_callback(_mediaPlayer, enhanceVideoFrameCallback, (__bridge_retained void *) self);
    } else {
        ijkmp_set_video_frame_callback(_mediaPlayer, NULL, NULL);
    }
}

- (BOOL) magicPixEnable {
    return _magicPixEnable;
}

- (void) putVideoFrame:(int)width h:(int)height frame:(uint8_t*)frame len:(int)length{
    NSLog(@"IJKFFMoviePlayerController, putVideoFrame, length:%d", length);
    ijkmp_put_video_frame(_mediaPlayer, width, height, (char*)frame, (int)length);
    frame = nil;
}
