...
public extension UIView {
     ...
     */
    func makeToast(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastPosition.center , title: String? = nil, image: UIImage? = nil, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
    ...
    /**
     ...
     @param position The toast's position
     */
    func makeToastActivity(_ position: ToastPosition = .center, title: String, showColse: Bool = true, bgEnable: Bool = true, bgColor: UIColor = UIColor(white: 0, alpha: 0.2), bgClickEnable: Bool = true, completion: @escaping (Bool) -> Void)  {
        
        guard objc_getAssociatedObject(self, &ToastKeys.activityView) as? UIView == nil else { return }
        
        let toast = createToastActivityView(hasCloseBtn : showColse)
        objc_setAssociatedObject(toast, &ToastKeys.completion, ToastCompletionWrapper(completion), .OBJC_ASSOCIATION_RETAIN_NONATOMIC)

        let point = position.centerPoint(forToast: toast, inSuperview: self)
        makeToastActivity(toast, point: point, bgEnable: bgEnable, bgColor: bgColor, bgClickEnable: bgClickEnable)
    }
    ...
    /**
     ...
     @param point The toast's center point
     */
    func makeToastActivity(_ point: CGPoint, showColse: Bool = true, bgEnable: Bool = false, bgColor: UIColor = UIColor(white: 0, alpha: 0.3), bgClickEnable: Bool = true, completion: @escaping (Bool) -> Void) {
        ...
        makeToastActivity(toast, point: point , bgEnable : bgEnable ,bgColor : bgColor )
    }
    
    /**
     Dismisses the active toast activity indicator view.
     */
    func hideToastActivity(duration: TimeInterval, block : @escaping ()->Void = {}) {
        
        if let toast = objc_getAssociatedObject(self, &ToastKeys.activityView) as? UIView {
            objc_removeAssociatedObjects(toast)

            UIView.animate(withDuration: duration, delay: 0.0, options: [.curveEaseIn, .beginFromCurrentState], animations: {
                toast.alpha = 0.0
                self.viewWithTag("toast".hashValue)?.alpha = 0
            }) { _ in
                toast.removeFromSuperview()
                self.viewWithTag("toast".hashValue)?.removeFromSuperview()
    ...
    func hideToastActivity(block : @escaping ()->Void = {}) {
    ...
    private func makeToastActivity(_ toast: UIView, point: CGPoint, showColse: Bool = false, closeActin : Selector? = nil, bgEnable: Bool = false, bgColor: UIColor = UIColor(white: 0, alpha: 0.0), bgClickEnable: Bool = true) {
        toast.alpha = 0.0
        toast.center = point
        ...
        if bgEnable {
            var bgView : A4xBaseToastBgView?
            bgView?.isUserInteractionEnabled = true
            bgView = A4xBaseToastBgView(frame: self.bounds)
            bgView?.onTopClickBlock = {[weak self] in
                if bgClickEnable {
                    self?.hideToastActivity()
                }
            }
            bgView?.tag = "toast".hashValue
            bgView!.backgroundColor = bgColor
            self.insertSubview(bgView!, belowSubview: toast)
        }
    ...
    private func createToastActivityView(title : String = "Loading",hasCloseBtn : Bool = false) -> UIView {
        let loadingV = UIImageView()
        loadingV.image = bundleImageFromImageName("home_video_loading")?.rtlImage()
        self.addSubview(loadingV)
        
        loadingV.size = CGSize(width: 30.auto() , height: 30.auto() )
        ...
        baseAnil.isRemovedOnCompletion = false

        baseAnil.repeatCount = MAXFLOAT
        loadingV.layer.add(baseAnil, forKey: "rotation")
        return loadingV
    }
    ...
    @objc
    private func closeActivityAction() {
        if let toast = objc_getAssociatedObject(self, &ToastKeys.activityView) as? UIView {
            if let wrapper = objc_getAssociatedObject(toast, &ToastKeys.completion) as? ToastCompletionWrapper, let completion = wrapper.completion {
                completion(true)
            }
    ...
    func toastViewForMessage(_ message: String?, title: String?, image: UIImage?, style: ToastStyle) throws -> UIView {
        ...
        if let messageLabel = messageLabel {
            ...
            if imageRect.size.width > 0 {
                ...
                imageRect.origin.y = messageRect.midY - imageRect.height / 2
                imageRect.origin.x = style.horizontalPadding
            }else {
                messageRect.origin.x = imageRect.origin.x + style.horizontalPadding
            }
        }
...
public struct ToastStyle {
     ...
     */
    public var activitySize = CGSize(width: 200.0, height: 60.0)
    
    public var activityInset : UIEdgeInsets = UIEdgeInsets(top: 10, left: 15, bottom: 10, right: 10)
    
    public var activityCloseSize = CGSize(width: 60.0, height: 60.0)
...
public class ToastManager {
     ...
     */
    public var position: ToastPosition = .bottom(offset:0)
