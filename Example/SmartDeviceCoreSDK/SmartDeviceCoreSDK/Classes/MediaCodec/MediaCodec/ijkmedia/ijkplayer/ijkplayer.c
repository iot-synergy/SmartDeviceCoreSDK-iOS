...
//a4x
void ijkmp_global_set_log_callback(ijk_log_callback cb) {
    ffp_global_set_log_callback(cb);
}
...
void ijkmp_set_option(IjkMediaPlayer *mp, int opt_category, const char *name, const char *value)
{
    ...
    MPTRACE("###ijkmp_set_option, %s(%s, %s)\n", __func__, name, value);
...
void ijkmp_set_option_int(IjkMediaPlayer *mp, int opt_category, const char *name, int64_t value)
{
    ...
    MPTRACE("####ijkmp_set_option_int, %s(%s, %"PRId64")\n", __func__, name, value);
    pthread_mutex_lock(&mp->mutex);
...
int ijkmp_set_data_source(IjkMediaPlayer *mp, const char *url)
{
    ...
    //const char* url = "http://192.168.31.54:8000/device_video_slice2/test.m3u8";
    pthread_mutex_lock(&mp->mutex);
...
{
    MPTRACE("ikjmp_chkst_start_l, mp_state:%d\n", mp_state);
...
int ijkmp_get_msg(IjkMediaPlayer *mp, AVMessage *msg, int block)
{
    ...
    while (1) {
        ...
        if (retval <= 0)
            return retval;
        MPTRACE("ijkmp_get_msg: msg->what：%d\n", msg->what);
        switch (msg->what) {
        ...
        case FFP_REQ_SEEK:
            ...
            break;
        case FFP_MSG_ERROR: {
            MPTRACE("ijkmp_get_msg: FFP_MSG_ERROR\n");
            mp->restart = 1;
            mp->restart_from_beginning = 1;
            ijkmp_change_state_l(mp, MP_STATE_ERROR);
            break;
...
void *ijkmp_set_video_frame_callback(IjkMediaPlayer *mp, ijk_video_frame_callback cb, void *opaque)
{
    assert(mp);
    MPTRACE("%s\n", __func__);
    void *old = NULL;
    pthread_mutex_lock(&mp->mutex);
    old = ffp_set_video_frame_callback(mp->ffplayer, cb, opaque);
    pthread_mutex_unlock(&mp->mutex);

    return old;
}

void ijkmp_put_video_frame(IjkMediaPlayer *mp, int w, int h, char *data, int len)
{
    assert(mp);
    MPTRACE("%s\n", __func__);
    pthread_mutex_lock(&mp->mutex);
    ffp_put_video_frame(mp->ffplayer, w, h, data, len);
    pthread_mutex_unlock(&mp->mutex);
}
