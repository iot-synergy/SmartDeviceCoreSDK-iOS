...
- (void)exportAsynchronouslyWithCompletionHandler:(void (^)(void))handler
{
    ...
    else
    {
        ...
        dispatch_async(queue, ^(void) {
            ...
            if(self.inputType == KMMediaAssetExportSessionInputTypeTS &&
               self.outputType == KMMediaAssetExportSessionOutputTypeMP4)
                NSLog(@"ts2Mp4, convertInputAssets start");
                [self convertInputAssets];
                NSLog(@"ts2Mp4, convertInputAssets done");
...
{
    NSLog(@"muxFilesFromTemporaryDirectory, inputUrl=%@, fps=%f", inputMuxDirectoryURL, video_stream_fps);
    NSError *error;
    ...
    if(!error)
    {
        ...
        //查找H264/H265文件
        if ([videoElementaryStreamFiles count] == 0) {
            videoExtensions = [NSArray arrayWithObjects:@"265", nil];
            videoElementaryStreamFiles = [inputMuxDirectoryContent filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"pathExtension IN %@", videoExtensions]];
        }
