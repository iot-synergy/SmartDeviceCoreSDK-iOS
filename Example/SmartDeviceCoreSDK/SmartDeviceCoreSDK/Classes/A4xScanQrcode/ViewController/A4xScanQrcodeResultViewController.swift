//
//  A4xScanQrcodeResultViewController.swift
//  AddxAi
//
//  Created by zhi kuiyu on 2019/5/8.
...
class A4xScanQrcodeResultViewController: A4xBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(deviceListChange(noti:)), name: A4xSaveKey.A4xDeviceListChangeNotificationKey, object: nil)
    }
    ...
    private func doneAction() {
        if A4xBaseConfig.shared.projectType == .APP {
            A4xAppRouter.router.open(fromClassName: "ScanQrcodeResultEnd")
        } else {
            let routerType = A4xRouterBaseType()
            routerType.moudleType = .deviceSetting
            routerType.funcType = .deviceShareScanQrcodeRequest
            routerType.pushType = .router
            A4xAppRouter.router.open(routerType: routerType,fromClassName: "ScanQrcodeResultEnd")
        }
    }
}
