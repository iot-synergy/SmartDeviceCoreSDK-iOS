...
#define FKLog(FORMAT, ...) NSLog(FORMAT,##__VA_ARGS__);
...
extern void checkURL(NSString * _Nullable address);
...
extern FKTaskInfoName const FKTaskInfoDescribe;
extern FKTaskInfoName const FKTaskInfoMediaType;
extern FKTaskInfoName const FKTaskInfoUserInfo;
...
typedef void(^FKTaskQueueUpdate) (NSArray<FKTask*> *tasks , int total , int wait, int complete , int error);  // 总进度变动 Block
NS_ASSUME_NONNULL_END


// ReactiveCocoa
typedef void (^rac_cleanupBlock_t)(void);
static inline void rac_executeCleanupBlock (__strong rac_cleanupBlock_t * _Nullable block) {
    (*block)();
}
